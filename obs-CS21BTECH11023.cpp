...
class MRMW_Snap {
public:
    MRMW_Snap(int size) : snap_size(size), array(size) {
    ...
    vector<int> snapshot() {
        vector<TimestampValue<int>> oldCopy = collect();
        while (true) {
            vector<TimestampValue<int>> newCopy = collect();
            if (consistent(oldCopy, newCopy)) {
                return taketheSanpshot(newCopy);
            }
            oldCopy = newCopy;
        }
    }
...
private:
    ...
    vector<TimestampValue<int>> collect() {
        vector<TimestampValue<int>> copy(snap_size);
    ...
    bool consistent(const vector<TimestampValue<int>>& oldCopy, const vector<TimestampValue<int>>& newCopy) {
        for (int i = 0; i < snap_size; ++i) {
...
void writer(MRMW_Snap& snap_obj, atomic<bool>& term, int thread_id, int M, double mu_w, 
    ...
    while (!term) {
        ...
        update_times.push_back(time_span);  // Recordinng the update time
        ...
        log_file << getCurrentTimestamp() << " - Thread " << thread_id << " wrote " << value << " at location " 
                 << loc <<endl;
...
void snapshot_thread(MRMW_Snap& snap_obj, int k, double mu_s, 
    ...
    for (int i = 0; i < k; ++i) {
        auto start = chrono::high_resolution_clock::now();
        vector<int> snap = snap_obj.snapshot();  // Collecting the snapshot
        auto end = chrono::high_resolution_clock::now();
        auto time_span = chrono::duration_cast<chrono::nanoseconds>(end - start).count();

        snapshot_times.push_back(time_span);  // Recording the snapshot time
        ...
        log_file << getCurrentTimestamp() << " - Snapshot " << i + 1 << " collected ";
        for (int val : snap) {
            log_file << val << " ";
        }
        log_file << endl;
